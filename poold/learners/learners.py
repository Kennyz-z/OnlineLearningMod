...
class AdaptiveDORM(OnlineLearner):
    """Adaptive‑DORM with GV/AdaGrad λ, Tsallis‑α, and regret smoothing."""
    ...
    def __init__(
        ...
        T: Optional[int] = None,
        adaptive_mode: str = "gv",           # "gv" | "adagrad"
        alpha: float = 0.5,                   # Tsallis‑α (0<α<1)
        beta: float = 0.98,                    # EWMA factor on regret
        eps: float = 1e-8,
        ridge: float = 1e-12,                 # avoid zero collapse
        g_scale_clip: float = 1e6,            # clip for G accumulation
    ) -> None:
        super().__init__(model_list, groups, T)
        if adaptive_mode not in {"gv", "adagrad"}:
            raise ValueError("adaptive_mode must be 'gv' or 'adagrad'.")
        alpha = min(alpha, 0.95)  # α→1 may explode
        if not (0.0 < alpha < 1.0):
            raise ValueError("alpha must be in (0,1).")
        if not (0.0 <= beta <= 1.0):
            raise ValueError("beta must be in [0,1].")
        self.alg = "AdaptiveDORM"
        self.mode = adaptive_mode
        self.alpha = alpha
        self.beta = beta
        self.eps = eps
        self.ridge = ridge
        self.g_scale_clip = g_scale_clip
        self.reset_params(T)
    ...
    def reset_params(self, T: Optional[int]):
        super().reset_params(T)
        self.w = self.init_weights()
        self.regret = np.zeros(self.d)
        self.G = np.full(self.d, self.eps)
        self.g_prev = np.zeros(self.d)  # GV baseline
    ...
    def update(
        ...
        if t_fb is None:
            self._apply_update(hint)
            return

        assert fb is not None and "g" in fb and "w" in fb
        g_fb = fb["g"].astype(float)
        w_fb = fb["w"]

        # Δ regret with grouping (q=2)
        delta_r = loss_regret(g_fb, w_fb, self.groups)
        self.regret = self.beta * self.regret + (1.0 - self.beta) * delta_r

        # second‑order accumulator
        if self.mode == "adagrad":
            self.G += np.minimum(delta_r ** 2, self.g_scale_clip)
        else:
            diff = g_fb - self.g_prev
            self.G += np.minimum(diff ** 2, self.g_scale_clip)
            self.g_prev = g_fb.copy()

        self._apply_update(hint)

    # -----------------------------------------------------------------
    def _apply_update(self, hint_vec: np.ndarray) -> None:
        lam = np.sqrt(self.G)
        z = (self.regret + hint_vec).clip(min=0.0) / lam + self.ridge
        if np.allclose(z, 0.0):
            self.w = self.init_weights()
            return

        z **= 1.0 / (1.0 - self.alpha)  # Tsallis‑α power
        self.w = normalize_by_groups(z, self.groups)
        if np.isnan(self.w).any():
            raise ValueError("NaNs in AdaptiveDORM weights.")