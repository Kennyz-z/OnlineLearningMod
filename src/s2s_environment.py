...
class S2SEnvironment(Environment):
    ...
    def __init__(self, times, models, gt_id, lead_time): 
        """ Initialize dataset.
        ...
        var = gt_id
        self.gt = get_ground_truth(gt_id)

        # Store delta between target date and forecast issuance date
        self.start_delta = timedelta(days=int(lead_time))
    ...
    def _get_loss(self, t):
        ...
        if self.gt_id=='precip':
            y_t = np.array(self.get_gt(t).tp).squeeze().flatten()
        elif self.gt_id=='tmp2m':
            y_t = np.array(self.get_gt(t).t2m).squeeze().flatten()
    ...
    def get_gt(self, t):
        ...
        return self.gt.sel(time=slice(target_str,target_str))
    ...
    def get_pred(self, t, verbose=False):
        ...
        for model in self.models:
            ...
            if merged_df is None:
                ...
                merged_df = np.hstack((merged_df,df))
    ...
    def get_model(self, t, model, verbose=False):
        ...
        df = ar.open_dataset(fname)
        df = df.sel(forecast_time0=slice(*(target_str,target_str)))
        if self.gt_id == 'precip':
            df = np.array(df.tp).squeeze()
        elif self.gt_id == 'tmp2m':
            df = np.array(df.t2m).squeeze()
