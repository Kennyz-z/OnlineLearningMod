...
from src.utils.ensembleprepare import last_issuance_date, get_issuance_dates, get_dates_from_filename
...
def get_forecast_filename(model="spatiotemporal_mean",
    ...
    Args:
       ...
       target_date_str: first date of target two week period; string in
         YYYYMMDD format
    """
    issuance_date = datetime.strptime(target_date_str,'%Y%m%d')-timedelta(days=int(lead_time))
    if gt_id=='precip':
        gt_id='prate'
    #file_folder=os.path.join("/home/climate/桌面/s2s/", model, gt_id)
    # file_folder=os.path.join("/mnt/e/s2s/",model,gt_id)
    file_folder = os.path.join(""
                               "/Users/skingz/online_suanfa/data/dataframes/CPSset/", model, gt_id)
    # print(model,gt_id,lead_time,target_date_str)
    file_open = last_issuance_date(get_dates_from_filename(file_folder),issuance_date)
    file_open_str = datetime.strftime(file_open,'%Y%m%d')
    return os.path.join(file_folder, f"{file_open_str}.nc")
