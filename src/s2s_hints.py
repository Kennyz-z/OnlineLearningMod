...
class S2SHinter(Hinter):
    def __init__(self, hint_types, gt_id, lead_time, learner, environment, 
    ...
    def get_day_offset(self, hint_key):
        ...
        if hint_key == "1day":
            ...
            return timedelta(days=int(self.lead_time))
...
class HorizonForecast(Hinter):
    ...
    def __init__(self, d, loss_gradient, loss_regret, environment, learner, model, gt_id, lead_time):
        # Base constructor
        super().__init__(d, loss_gradient, loss_regret, environment, learner)
        self.model = model
        self.lead_time = lead_time
    ...
    def get_pseudo_grad(self, t, ind=None, D=None): 
        ...
        if not os.path.exists(fname):
            printf(f"Warning: No {self.model} forecast for lead time {self.lead_time} on date {date_str}") 
            return np.zeros((self.d,))
